...
2、Spring是一个轻量级的框架 , 非侵入式的 (对原来栏目不影响).
...
- 之前，是程序主动创建对象，控制权在程序员手里
- 现在，使用set注入后，程序不再具有主动性，而是变成了被动的接受对象

大家发现了区别没有 ? 可能很多人说没啥区别 . 但是同学们 , 他们已经**发生了根本性的变化** , 很多地方都不一样了 . 仔细去思考一下 , 以前所有东西都是由程序去进行控制创建 , 而现在是由我们自行控制创建对象 , 把**主动权交给了调用者** . 程序不用去管怎么创建,怎么实现了 . 它只负责提供一个接口 .
